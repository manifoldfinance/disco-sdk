...
import { PlainObject, DbObject, AnyObject, Primitive } from "./object";
...
export class DbObjectSerialiser {
    ...
    private serialiseAnyObject(obj: AnyObjectOrSerialisable): AnyObject {
        if (isPrimitive(obj)) {
            ...
            if(result.__type__ && !this.deserialisers[result.__type__]) throw new VError(`Tried to serialise an object with __type__ ${result.__type__}, but no deserialiser is set for it. Make sure the serialiser is constructed with all the correct deserialisers.`); // prettier-ignore
    ...
    private deserialiseAnyObject<T>(obj: AnyObject): T {
        if (isPrimitive(obj)) {
            ...
            if (this.deserialisers[type]) {
                ...
                throw new VError(`EXCEPTION type while deserialising: ${type}`);
            }
        } else {
...
export class SerialisableBigNumber extends BigNumber implements Serialisable {
    ...
    public static deserialise(obj: Serialised<SerialisableBigNumber>): SerialisableBigNumber {
        if (obj.__type__ !== SerialisableBigNumber.TYPE) throw new ApplicationError(`EXCEPTION __type__ while deserialising SerialisableBigNumber: ${obj.__type__}`); // prettier-ignore
